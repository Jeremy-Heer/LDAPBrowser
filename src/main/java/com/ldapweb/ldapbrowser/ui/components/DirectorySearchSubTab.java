...
/**
 * Directory Search sub-tab for searching users, groups, and all entries across
...
public class DirectorySearchSubTab extends VerticalLayout {
  ...
  /**
   * Constructs a new DirectorySearchSubTab with the required services.
   *
   * @param ldapService the LDAP service for performing searches
   * @param configurationService the configuration service for server settings
   * @param inMemoryLdapService the in-memory LDAP service for testing
   * @param loggingService the logging service for recording operations
   */
  ...
  private void initializeResultsComponents() {
      ...
      checkbox.addValueChangeListener(event -> {
        if (event.getValue()) {
          if (selectedForComparison.size() < 10) {
            ...
            showError("You can select up to 10 entries "
                + "for comparison");
            return;
          }
        } else {
    ...
    resultsGrid.addColumn(
        entry -> String.join(", ", entry.getAttributeValues("objectClass")))
  ...
  /**
   * Updates the search buttons based on current input and environment selection.
   */
    ...
    if (searchButton != null) {
      ...
      if (loggingService != null && loggingService.isDebugCaptureEnabled()) {
        if (!enabled) {
          ...
          loggingService.logDebug("UI/SEARCH", "Basic Search button enabled=" + enabled,
              missing.toString());
        } else {
      ...
      if (loggingService != null && loggingService.isDebugCaptureEnabled()) {
        if (!enabled) {
          ...
          if (filter == null || filter.trim().isEmpty()) {
            missing.append("filter empty; ");
          }
          loggingService.logDebug(
                "UI/SEARCH",
                "Advanced Search button enabled=" + enabled,
                missing.toString()
          );
        } else {
    ...
    try {
      ...
      for (LdapServerConfig environment : selectedEnvironments) {
        ...
        try {
          ...
          String msg = "Failed to connect to environment '" + environment.getName()
              + "': " + ex.getMessage();
        ...
        try {
          ...
          String errorMsg = "Search failed for environment '"
    ...
    try {
      ...
      for (LdapServerConfig environment : selectedEnvironments) {
        ...
        try {
          ...
          showError("Failed to connect to environment " + environment.getName()
              + ": " + ex.getMessage());
          continue; // Skip this environment
        }
        ...
        if (searchBase == null || searchBase.trim().isEmpty()) {
          showError("No base DN configured for environment: "
              + environment.getName());
          continue;
        }
        ...
        try {
          ...
          showError("Search failed for environment "
  ...
  private void displayResults(List<SearchResultEntry> results,
      String searchDescription) {
    // If we have a search results callback, use it to display results in the
    // dedicated tab
    if (searchResultsCallback != null) {
      ...
      if (results.isEmpty()) {
        ...
        showSuccess(String.format("Found %d %s", results.size(),
  ...
  /**
   * Refreshes the environment settings and updates search buttons.
   */
  ...
  private void showEntryDetailsDialog(LdapEntry entry,
      LdapServerConfig environment) {
    Dialog dialog = new Dialog();
    dialog.setHeaderTitle("Entry Details - "
        + environment.getName());
    ...
    try {
      LdapService.EntryWithSchema entryWithSchema =
          ldapService.getEntryWithSchema(environment.getId(), entry.getDn());
      if (entryWithSchema != null) {
        attributeEditor.editEntryWithSchema(
            entryWithSchema.getEntry(),
      ...
      if (component instanceof HorizontalLayout) {
        ...
        layout.getChildren().forEach(child -> {
          if (child instanceof Button) {
            ...
            if (text != null && (text.contains("Save") || text.contains("Add")
   ...
   */
  public void setComparisonCallback(
      Consumer<List<SearchResultEntry>> callback) {
   ...
   */
  public void setSearchResultsCallback(
      java.util.function.BiConsumer<List<SearchResultEntry>, String> callback) {

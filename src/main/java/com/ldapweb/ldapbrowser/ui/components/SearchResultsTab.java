...
/**
 * Search Results tab for displaying search results from both basic and advanced
 * searches.
 */
public class SearchResultsTab extends VerticalLayout {
  ...
  /**
   * Constructs a new {@code SearchResultsTab} instance.
   * Initializes UI components and sets up the layout for displaying LDAP search results.
   *
   * @param ldapService the service used to perform LDAP operations and retrieve search results
   */
    ...
    selectAllCheckbox.addValueChangeListener(event -> {
      ...
      if (selectAll) {
        ...
        if (toSelect < visibleEntries.size()) {
          showError(
              "You can select up to 10 entries for comparison. Selected first 10 entries.");
        }
      } else {
        selectedForComparison.clear();
      }
  ...
  private void viewEntry(SearchResultEntry entry) {
    try {
      ...
      try {
        LdapService.EntryWithSchema entryWithSchema = ldapService.getEntryWithSchema(
            environment.getId(),
            entry.getEntry().getDn());

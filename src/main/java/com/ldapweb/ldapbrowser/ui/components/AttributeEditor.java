...
public class AttributeEditor extends VerticalLayout {
  ...
  private void refreshAttributeDisplay() {
    ...
    // Sort attributes according to priority: objectClass > required > optional >
    // operational
  ...
  /**
   * Comprehensive check for operational attributes using both pattern matching
   * and schema
   */
  ...
  private String getEntryTypeDescription(LdapEntry entry) {
    ...
    if (objectClasses != null && !objectClasses.isEmpty()) {
      ...
      // If no specific match found, return the most specific objectClass (usually the
      // first one)
  ...
  private void openAddAttributeDialog() {
    ...
    Button saveButton = new Button("Add", e -> {
      ...
      if (existingValues != null && !existingValues.isEmpty()) {
        ...
        markPendingChanges();
        showSuccess("Added " + (mergedValues.size() - existingValues.size()) + " new value(s) to existing attribute '"
            + attributeName + "'.");
      } else {
  ...
  /**
   * Special dialog for editing objectClass attribute with type-ahead
   * functionality
   */
  ...
  private void copySearchFilter(AttributeRow row) {
    // For multi-valued attributes, create filters for each value and combine them
    // with OR
    ...
    getUI().ifPresent(ui -> ui.getPage().executeJs("navigator.clipboard.writeText($0)", filter));
  ...
  private void copyValues(AttributeRow row) {
    ...
    // Copy to clipboard using JavaScript
    getUI().ifPresent(ui -> ui.getPage().executeJs("navigator.clipboard.writeText($0)", valueText));

    // Show notification
    String message = values.size() == 1 ? "Attribute value copied to clipboard"
  ...
  private void copyDnToClipboard() {
    ...
    // Copy to clipboard using JavaScript
    getUI().ifPresent(ui -> ui.getPage().executeJs("navigator.clipboard.writeText($0)", dn));

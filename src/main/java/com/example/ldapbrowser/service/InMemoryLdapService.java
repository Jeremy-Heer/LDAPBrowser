...
public class InMemoryLdapService {
  ...
  public List<LdapServerConfig> getAllInMemoryServers() {
    ...
    // This handles cases where DevTools recreated the Bean but lost the in-memory
    // state
    File configFile = new File(CONFIG_FILE_PATH);
  ...
  public void addObjectClassToSchema(String serverId, String objectClassDefinition) throws LDAPException {
    ...
    try {
      // For in-memory servers, we can add schema elements through server
      // configuration
      // This is a simplified approach that validates the definition
      @SuppressWarnings("unused")
      com.unboundid.ldap.sdk.schema.ObjectClassDefinition objectClass = new com.unboundid.ldap.sdk.schema.ObjectClassDefinition(
          objectClassDefinition);

      // If parsing succeeds, the definition is valid
      // Note: For full schema updates, the server would need to be restarted with new
      // schema
  ...
  public void addAttributeTypeToSchema(String serverId, String attributeTypeDefinition) throws LDAPException {
    ...
    try {
      ...
      // This is a simplified approach that validates the definition
      @SuppressWarnings("unused")
      com.unboundid.ldap.sdk.schema.AttributeTypeDefinition attributeType = new com.unboundid.ldap.sdk.schema.AttributeTypeDefinition(
